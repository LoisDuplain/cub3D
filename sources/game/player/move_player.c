...
/*   Updated: 2021/04/27 16:42:22 by lduplain         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void	move_player(t_game *game, float vx, float vy)
{
	...
	nx = px + vx;
	ny = py + vy;
	if (!sprite_collide_player(game, nx, py)
		&& game->cur_lvl->map_content[(int)(py)][(int)(nx)] != 1)
		game->cur_lvl->player.position.vx = nx;
	if (!sprite_collide_player(game, px, ny)
		&& game->cur_lvl->map_content[(int)(ny)][(int)(px)] != 1)
