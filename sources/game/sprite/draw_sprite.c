	...
	t_ray ray, t_sprite sprite)
{
	...
	distance = get_sprite_distance(sprite, ray, r_thread->lvl->player);
	if (distance <= 0 || distance > r_result->distance
		|| distance > r_thread->lvl->player.render_distance)
		return ;
	intrsct = get_sprite_intrsct_point(r_thread->lvl->player, ray,
			distance);
	ratio = get_sprite_ratio(intrsct, sprite, r_thread->lvl->player);
	if (ratio < 0 || ratio > 1)
		return ;
	...
	bettermlx_pixel_put(r_thread->window, ray.pixel, color,
		1 - distance / r_thread->lvl->player.render_distance);
}
